...
fmnist$train$x <- (fmnist$train$x - 127.5)/127.5
fmnist$test$x <- (fmnist$test$x - 127.5)/127.5
fmnist$train$x <- array_reshape(fmnist$train$x, c(60000, 1, 28, 28))
fmnist$test$x <- array_reshape(fmnist$test$x, c(10000, 1, 28, 28))

num_train <- dim(fmnist$train$x)[1]
num_test <- dim(fmnist$test$x)[1]
...
for(epoch in 1:epochs){
  ...
  for(index in 1:num_batches){
    ...
    image_batch <- fmnist$train$x[batch,,,,drop = FALSE]
    label_batch <- fmnist$train$y[batch]
  ...
  X <- abind(fmnist$test$x, generated_images, along = 1)
  y <- c(rep(1, num_test), rep(0, num_test)) %>% matrix(ncol = 1)
  aux_y <- c(fmnist$test$y, sampled_labels) %>% matrix(ncol = 1)
...
#Saving the model
generator %>% save_model_hdf5("generator_acgan_fashion_mnist_r.h5")
discriminator %>% save_model_hdf5("discriminator_acgan_fashion_mnist_r.h5")

#Restoring the model
load_generator <- load_model_hdf5("generator_acgan_fashion_mnist_r.h5")
load_discriminator <- load_model_hdf5("discriminator_acgan_fashion_mnist_r.h5")
